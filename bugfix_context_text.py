...
# 10 varied, iterative, and detailed code-fixing prompts
debugging_promptss = [
    " ",
...
# OpenAI API key (set your key here or use env variable)
load_dotenv()
...
# Helper: check if test passes
def test_passes(returncode, stdout, stderr):
    # A proper check for pytest success:
    # 1. Return code should be 0 for successful tests
    # 2. Check for both "FAILED" not in output and positive indicators of success
...
for i, system_prompt in enumerate(system_prompts):
    ...
    bug_message = None  # No static message, will be set from test output
    for attempt in range(1, 4):
        ...
        if passed:
            ...
            break
        # Always use actual test output as bug_message
        print(out, err)
        bug_message = f"Test output (stdout):\n{out}\nTest output (stderr):\n{err}"
        # Call GPT with system prompt
        try:
        ...
        prompt_result["logs"].append({
            ...
            "post_fix_stdout": out,
            "post_fix_stderr": err,
            "model_output_file": fixed_code  # Save model output
        })
    ...
    if not prompt_result["success"]:
        prompt_result["iterations"] = 5
    results.append(prompt_result)
    # Always reset before next prompt
    run_cmd(RESET_CMD)
    time.sleep(2)
...
iterations = [(r["iterations"]-1)/2 for r in results]
...
# Annotate bars with number of iterations and pass/fail
